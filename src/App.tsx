...
const SchedulePlanner: React.FC = () => {
  ...
  const validateForm = (): boolean => {
    ...
    const emptySubjects = formData.subjects.some(subject => !subject.name.trim());
    if (emptySubjects) {
      errors.push('All subjects must have a name');
    }
    
    // Check for duplicate subject names
    const subjectNames = formData.subjects.map(subject => subject.name.trim().toLowerCase());
      ...
      <main className="container mx-auto px-4 py-8 max-w-4xl">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-8">
          <div className="bg-blue-50 p-5 border-b border-blue-100">
            <h2 className="text-xl font-semibold text-blue-900">Study Preferences</h2>
            <p className="text-blue-700 text-sm mt-1">Add your subjects and set your daily study hours</p>
          </div>
              ...
              <div className="space-y-3 mb-4">
                {formData.subjects.map((subject, index) => (
                  <SubjectField
                    ...
                    isRemovable={formData.subjects.length > 3}
                  />
                ))}
              </div>
